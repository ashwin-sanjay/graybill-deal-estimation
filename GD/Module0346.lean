...
namespace GD.N0106.N0428.N0764
...
theorem d005147 {F F' : ℂ → X → ℂ} {U : Set ℂ} {M : ℂ → ℝ}
    (hU : IsOpen U) (hM : ContinuousOn M U)
    (hm : ∀ z ∈ U, AEStronglyMeasurable (F z) μ)
    (hm' : ∀ z ∈ U, AEStronglyMeasurable (F' z) μ)
    (hF : ∀ᵐ x ∂μ, ∀ z ∈ U,
      HasDerivAt (fun w => F w x) (F' z x) z ∧ ‖F z x‖ ≤ M z)
    {z : ℂ} (hz : z ∈ U) :
    Integrable (F' z) μ ∧
  ...
  have hb : ∀ᵐ x ∂μ, ∀ w ∈ closedBall z r, ‖F' w x‖ ≤ C / r := by
    ...
    have hc := Complex.norm_deriv_le_of_forall_mem_sphere_norm_le hr
      ...
      (fun v hv => ((hx v (hRU (hsub hw (sphere_subset_closedBall hv)))).2).trans
        ((le_abs_self (M v)).trans (hC v (hsub hw (sphere_subset_closedBall hv)))))
    rwa [(hx w (hs hw)).1.deriv] at hc
  have hi : Integrable (F z) μ := (integrable_const (M z)).mono' (hm z hz)
  ...
  exact hasDerivAt_integral_of_dominated_loc_of_deriv_le
    ...
    hb (integrable_const (C / r))
    (hF.mono fun x hx w hw => (hx w (hs hw)).1)

end
end GD.N0106.N0428.N0764
#print axioms _root_.GD.N0106.N0428.N0764.d005147
